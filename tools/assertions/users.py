...
def assert_create_user_response(request: CreateUserRequestSchema, response: CreateUserResponseSchema):
    ...
    :param response: Ответ API на создание пользователя
...
def assert_get_user_response(get_user_response: GetUserResponseSchema, create_user_response: CreateUserResponseSchema):
    # В assert_user передаем ответ в виде модели UserSchema, т.е. проваливаемся в объект user
    return assert_user(get_user_response.user, create_user_response.user)